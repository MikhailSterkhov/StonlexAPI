package ru.stonlex.bungee.inventory.item.material;
...
public enum BungeeMaterial {
    ...
    private static BungeeMaterial[] byId = new BungeeMaterial[values().length];
    private static final Map<String, BungeeMaterial> BY_NAME = Maps.newHashMap();
    ...
    static {
        ...
        for (int var1 = 0; var1 < var2; ++var1) {
            BungeeMaterial bungeeMaterial = var3[var1];
            if (byId.length > bungeeMaterial.id) {
                byId[bungeeMaterial.id] = bungeeMaterial;
            } else {
                byId = Arrays.copyOfRange(byId, 0, bungeeMaterial.id + 2);
                byId[bungeeMaterial.id] = bungeeMaterial;
            }

            BY_NAME.put(bungeeMaterial.name(), bungeeMaterial);
        }

    }

    BungeeMaterial(int id) {
        this(id, 64);
    }

    BungeeMaterial(int id, int stack) {
        this(id, stack, 0);
    }

    BungeeMaterial(int id, int stack, int durability) {
        this.id = id;
    ...
    public static BungeeMaterial getMaterial(int id) {
        return byId.length > id && id >= 0 ? byId[id] : null;
    }

    public static BungeeMaterial getMaterial(String name) {
        return BY_NAME.get(name);
    }

    public static BungeeMaterial matchMaterial(String name) {
